...
(function() {
  ...
  // Session handling (tab-scoped, resets per visit)
  ...
  // Visitor handling (persistent across sessions via localStorage)
  function getVisitorId() {
    let vid = null;
    try {
      vid = localStorage.getItem('plots_vid');
      if (!vid) {
      ...
      vid = getSessionId(); // fallback to session
    }
    return vid;
  }
